...
que puede ser pendiente, cumplida o abandonada. Además nos avisan que existen misiones difíciles, que son
...
\subsection{Clases a crear}
Creamos la clase Mision, 
\subsection{Iniciamos un workspace}
Abrimos un Workspace de trabajo y vamos a inicializar un juego de variables nuevo:
\begin{lstlisting}[frame=single]
diego := Heroe new
heroe agregarMision: (BuscarItemMagico new)
\end{lstlisting}
...
devuelve información, entonces elegimos la opción \textit{Display It}:
\begin{lstlisting}[frame=single]
diego cantidadMisiones
\end{lstlisting}
...
opción hay una traducción implícita: size > 0.... ah, es si tiene elementos. Es un detalle, pero un detalle que
